@foreach($counties as $county)
    @php
        $isArr = is_array($county);
        $slug = $isArr ? ($county['slug'] ?? '') : $county->slug;
        $name = $isArr ? ($county['name'] ?? '') : $county->name;
        $count = $isArr ? ($county['businesses_count'] ?? 0) : $county->businesses_count;
        $img = $isArr ? ($county['display_image_url'] ?? asset('images/placeholder-county.jpg')) : ($county->display_image_url ?? asset('images/placeholder-county.jpg'));
    @endphp

    <a href="{{ route('listings.county', ['countySlug' => $slug]) }}" class="county-card-link" style="text-decoration: none; color: inherit; display: block;">

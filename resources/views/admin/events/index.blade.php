<x-admin.layouts.app>
    ...
    <div x-data="{
        ...
        fetchResults(url = null) {
            ...
            const fetchUrl = url || `{{ route('admin.events.index') }}?${params.toString()}`;
    ...
    class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="relative flex-1 max-w-sm">
                <input type="text"
                    ...
                    class="block w-full pl-3 pr-10 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Search events...">
             ...
             <div class="flex items-center gap-2">
                 {{-- Events often created by frontend users, but admin can create too --}}
            </div>
        </div>
        ...
        <div x-ref="results" class="overflow-x-auto" @click="if($event.target.tagName === 'A' && $event.target.closest('nav')) { $event.preventDefault(); fetchResults($event.target.href); }">
            @include('admin.events._table', ['events' => $events])
        </div>
    </div>
</x-admin.layouts.app>
